...
import { ChartSavedObjectAttributes, VisTypeNiitChartPluginSetupDependencies } from '../types';

type UsageCollectorDependencies = Pick<VisTypeNiitChartPluginSetupDependencies, 'home'>;

type ChartType = 'niit_chart' | 'niit_chart-lite';

function isChartType(attributes: any): attributes is ChartSavedObjectAttributes {
  return attributes && attributes.type === 'niit_chart' && attributes.params?.spec;
}

const checkChartSchemaType = (schemaURL: string, type: ChartType) =>
  schemaURL.includes(`//niit_chart.github.io/schema/${type}/`);
...
export const getStats = async (
  ...
  const niit_chartUsage = {
    niit_chart_lib_specs_total: 0,
    niit_chart_lite_lib_specs_total: 0,
    niit_chart_use_map_total: 0,
  };
    ...
    if (isChartType(visState) && !excludedFromStatsVisualizations.includes(visState.title)) {
      try {
        ...
        if (spec) {
          ...
          if (checkChartSchemaType(spec.$schema, 'niit_chart')) {
            niit_chartUsage.niit_chart_lib_specs_total++;
          }
          if (checkChartSchemaType(spec.$schema, 'niit_chart-lite')) {
            niit_chartUsage.niit_chart_lite_lib_specs_total++;
          }
          if (spec.config?.kibana?.type === 'map') {
            niit_chartUsage.niit_chart_use_map_total++;
          }
        }
      } catch (e) {
  ...
  return shouldPublishTelemetry ? niit_chartUsage : undefined;
};
