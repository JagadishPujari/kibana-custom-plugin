...
import { ConfigObservable, VisTypeNiitChartPluginSetupDependencies } from '../types';

export function registerChartUsageCollector(
  collectorSet: UsageCollectionSetup,
  config: ConfigObservable,
  dependencies: Pick<VisTypeNiitChartPluginSetupDependencies, 'home'>
) {
  const collector = collectorSet.makeUsageCollector<ChartUsage | undefined>({
    type: 'vis_type_niit_chart',
    isReady: () => true,
    schema: {
      niit_chart_lib_specs_total: { type: 'long' },
      niit_chart_lite_lib_specs_total: { type: 'long' },
      niit_chart_use_map_total: { type: 'long' },
    },
