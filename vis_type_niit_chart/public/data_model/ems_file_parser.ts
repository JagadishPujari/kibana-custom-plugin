...
export class EmsFileParser {
  ...
  parseUrl(obj: Data, url: UrlObject) {
    if (typeof url.name !== 'string') {
      throw new Error(
        i18n.translate('visTypeNiitChart.emsFileParser.missingNameOfFileErrorMessage', {
          defaultMessage:
  ...
  async populateData(requests: EmsQueryRequest[]) {
    ...
    for (const { obj, name } of requests) {
      ...
      if (!foundLayer) {
        throw new Error(
          i18n.translate('visTypeNiitChart.emsFileParser.emsFileNameDoesNotExistErrorMessage', {
