...
export type InspectDataSets = Assign<ChartRuntimeData, { id: string }>;
export type InspectSignalsSets = ChartRuntimeData;

const chartAdapterSignalLabel = i18n.translate('visTypeChart.inspector.chartAdapter.signal', {
  defaultMessage: 'Signal',
});

const chartAdapterValueLabel = i18n.translate('visTypeChart.inspector.chartAdapter.value', {
  ...
  getSignalsSetsSubscription(): Observable<InspectSignalsSets> {
    ...
    return merge(this.debugValuesSubject, signalsListener).pipe(
      ...
      map((debugValues) => {
        ...
        return {
          columns: [
            { id: chartAdapterSignalLabel, schema: 'text' },
            { id: chartAdapterValueLabel, schema: 'json' },
          ],
