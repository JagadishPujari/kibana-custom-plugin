...
for (const funcName of Object.keys(chartFunctions)) {
  if (!expressionFunction(funcName)) {
    expressionFunction(funcName, function handlerFwd(...args) {
      const view = this.context.dataflow;
      view.runAfter(() => view._kibanaView.chartFunctionsHandler(funcName, ...args));
    });
  }
}
...
export class ChartBaseView {
  constructor(opts) {
    ...
    this._view = null;
    this._chartViewConfig = null;
    this._$messages = null;
    this._destroyHandlers = [];
    this._initialized = false;
    this._enableExternalUrls = getEnableExternalUrls();
    this._chartStateRestorer = opts.chartStateRestorer;
  }
    ...
    try {
      ...
      this._addDestroyHandler(() => {
        ...
        if (this._view) {
          ...
          if (state) {
            this._chartStateRestorer.save(state);
          }
          this._view.finalize();
        }
        this._view = null;
      ...
      this._chartViewConfig = this.createViewConfig();
  ...
  async findIndex(index) {
    ...
    if (index) {
      ...
      if (!idxObj) {
        throw new Error(
          i18n.translate('visTypeChart.chartParser.baseView.indexNotFoundErrorMessage', {
      ...
      if (!idxObj) {
        ...
        if (defaultIdx) {
          ...
          throw new Error(
            i18n.translate('visTypeChart.chartParser.baseView.unableToFindDefaultIndexErrorMessage', {
  ...
  createViewConfig() {
    ...
    const chartLoader = loader();
    const originalSanitize = chartLoader.sanitize.bind(chartLoader);
    chartLoader.sanitize = async (uri, options) => {
      if (uri.bypassToken === bypassToken) {
        ...
        throw new Error(
          i18n.translate('visTypeChart.chartParser.baseView.externalUrlsAreNotEnabledErrorMessage', {
            defaultMessage:
              'External URLs are not enabled. Add   {enableExternalUrls}   to {kibanaConfigFileName}',
            values: {
              enableExternalUrls: 'vis_type_chart.enableExternalUrls: true',
    ...
    config.loader = chartLoader;

    const chartLogger = logger(Warn);

    chartLogger.warn = this.onWarn.bind(this);
    chartLogger.error = this.onError.bind(this);

    config.logger = chartLogger;
  ...
  setView(view) {
    ...
    if (view) {
      // Global chart expression handler uses it to call custom functions
      ...
      const state = this._chartStateRestorer.restore();
   ...
   */
  async chartFunctionsHandler(funcName, ...args) {
    try {
      const handlerFunc = chartFunctions[funcName];
      if (!handlerFunc || !this[handlerFunc]) {
        // in case functions don't match the list above
        throw new Error(
          i18n.translate(
            'visTypeChart.chartParser.baseView.functionIsNotDefinedForGraphErrorMessage',
            {
  ...
  static _parseTimeRange(start, end) {
    ...
    if (isValidAbsStart && isValidAbsEnd) {
      ...
      if (!startDate || !endDate || !startDate.isValid() || !endDate.isValid()) {
        throw new Error(
          i18n.translate('visTypeChart.chartParser.baseView.timeValuesTypeErrorMessage', {
            defaultMessage:
  ...
  /**
   * Set global debug variable to simplify chart debugging in console. Show info message first time
   */
  setDebugValues(view, spec, vlspec) {
    this._parser.searchAPI.inspectorAdapters?.chart.bindInspectValues({
      view,
      spec: vlspec || spec,
    });
    ...
    if (window) {
      if (window.JAGS_DEBUG === undefined && console) {
        ...
        console.log(
          'You can access the Chart view with JAGS_DEBUG. ' +
            'Learn more at https://vega.github.io/vega/docs/api/debugging/.'
        );
      }
      const debugObj = {};
      window.JAGS_DEBUG = debugObj;
      window.JAGS_DEBUG.JAGS_VERSION = vegaVersion;
      window.JAGS_DEBUG.JAGS_LITE_VERSION = vegaLiteVersion;
      window.JAGS_DEBUG.view = view;
      window.JAGS_DEBUG.chart_spec = spec;
      window.JAGS_DEBUG.chartlite_spec = vlspec;
