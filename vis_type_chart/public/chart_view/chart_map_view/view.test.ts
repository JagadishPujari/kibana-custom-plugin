...
import { ChartMapView } from './view';
import { ChartViewParams } from '../chart_base_view';
...
import { initChartLayer, initTmsRasterLayer } from './layers';
...
describe('chart_map_view/view', () => {
  describe('ChartMapView', () => {
    ...
    async function createChartMapView() {
      await chartParser.parseAsync();
      return new ChartMapView(({
        chartParser,
    ...
    test('should be added TmsRasterLayer and do not use tmsService if mapStyle is "user_configured"', async () => {
      ...
      const chartMapView = await createChartMapView();
      ...
      expect(initChartLayer).toHaveBeenCalled();
    });
